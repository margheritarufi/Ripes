...
CacheConfigWidget::CacheConfigWidget(QWidget *parent)
  ...
  //rufi
  typeOfCache = parent; //The parent of typeOfCache is either a QWidget tab (data cache) or QWidget tab_2 (instruction cache)
...
void CacheConfigWidget::setCache(const std::shared_ptr<CacheSim> &cache) {
  ...
  if(typeOfCache->parent()->objectName().compare("tab", Qt::CaseInsensitive) == 0){
  qDebug() << "Initial number of ways of data cache: " << m_cache->getWaysBits();
  qDebug() << "Initial number of lines of data cache: " << m_cache->getLineBits();
  qDebug() << "Initial number of blocks of data cache: " << m_cache->getBlockBits();
  saveNbWaysDataCache(m_cache);
  saveNbLinesDataCache(m_cache);
  saveNbBlocksDataCache(m_cache);
  }

  if(typeOfCache->parent()->objectName().compare("tab_2", Qt::CaseInsensitive) == 0){
  qDebug() << "Initial number of ways of instr cache: " << m_cache->getWaysBits();
  qDebug() << "Initial number of lines of instr cache: " << m_cache->getLineBits();
  qDebug() << "Initial number of blocks of instr cache: " << m_cache->getBlockBits();
